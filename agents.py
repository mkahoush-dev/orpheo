...
class ToolCallingAgent:
    """
    Based on the `Build Your Own OpenAI agent` blog post from:
    ...
    def __init__(
        ...
        if llm: 
            ...
            self._llm, _ = get_model_from_name(llm_name)
...
class MultiDocumentAgents():
    ...
    def _compose_query_engines_and_agents(self):
        directory_path = './data'
        all_files = self.list_all_files(directory_path)
        ...
        for file_path in self.file_paths:
            ...
            self.query_engines[file_title] = vector_index.as_query_engine(similarity_top_k=2)
        if self.all_tools == []:
